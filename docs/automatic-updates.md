...
GameTest 使用候選版本的實際 KRC 驅動器註冊表，檢查完整變身、權限、傷害倍率、重複套用、血量比例、登出與停用行為。若 KRC 改動 Java 介面、裝備或相依需求導致編譯或測試失敗，工作流程會失敗並保留診斷資料，等待程式碼相容修正。

這個流程追蹤現有 Minecraft 1.21.1 分支。移植到其他 Minecraft／載入器版本仍需相應開發與驗證。

## 版本與重試

例如以 KRC 版本 ID `I61dxvsY` 建置時，產物版本為 `1.0.0+krc.I61dxvsY`，標籤為 `compat-v1.0.0-krc-I61dxvsY`。正式版 `v1.0.0` 保持獨立。自動相容性版本標為 prerelease，並在說明中列出實際測試的 KRC 版本。

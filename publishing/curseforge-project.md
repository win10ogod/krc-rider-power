...
| Summary | Shared Rider Power for Kamen Rider Craft: good deeds strengthen bonuses, while evil karma trades health for damage. |
| Class | Mods |
...
| Screenshot caption | Configure the six shared base bonuses; merit and evil karma determine power, damage, and maximum health. The current editor uses Traditional Chinese. Saving requires server operator permission level 2. |

## 上傳檔案

| 欄位 | 填入內容 |
| --- | --- |
| Upload file | `krc-rider-power-1.2.0.jar`，位於本地 `dist/1.2.0/` 或 GitHub Release |
| Display name | KRC Rider Power 1.2.0 — NeoForge 1.21.1 |
| Release type | Release |
| Game version | 1.21.1 |
| Mod loader | NeoForge |
| Java version | Java 21（若表單提供此欄位） |
| Changelog | 使用 [CHANGELOG.md](../CHANGELOG.md) 的 1.2.0 段落 |
